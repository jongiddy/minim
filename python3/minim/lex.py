...
class token_types:
    ...
    def get_token(self, token_type):
        ...
        method will allocate and return an actual token, which can be
        processed further.
    ...
    def set_token(self, token_type, token):
        ...
        This function differs from ``get_token`` in that it may choose
        to use the provided (subclass of) ``Token`` as the returned
        token.  This allows the opportunity for the system to avoid
        allocating memory for the token.

        Note that this method is not required to use the provided token,
        so the return value must be used for processing the current
        token.

        :param token_type: A Token Type returned from the iterator
        :param token: A Token that can be set
